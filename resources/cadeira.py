...
# ALTERAÇÕES
class CadeiraId(Resource):
    @classmethod
    @jwt_optional
    def get(cls, cadeira_id: int):
        cadeira = CadeiraModel.find_by_id(cadeira_id)
        if cadeira:
            return cadeira_schema.dump(cadeira), 200
        return {"message": gettext("cadeira_not_found")}, 404
    ...
    def put(cls, cadeira_id: int):
        ...
        if cadeira:
            cadeira_json = request.get_json()
            cadeira.curso_id = cadeira_json["curso_id"]
            cadeira.name = cadeira_json["name"]
            cadeira.save_to_db()
            return cadeira_schema.dump(cadeira), 200
        else:
            return {"message": gettext("cadeira_not_found")}, 404
...
class CadeiraGeral(Resource):
    @classmethod
    @jwt_optional
    def get(cls):
        if len(request.args) == 0 or (request.args.get("search") is None and request.args.get("search") is None):
            return {"cadeiras": cadeira_list_schema.dump(CadeiraModel.find_all())}, 200
        else:
            return {"cadeiras": cadeira_list_schema.dump(CadeiraModel.find_all(request.args['search'],
                                                                               request.args['curso']))}, 200
    ...
    def post(cls):
        ...
        if CadeiraModel.find_by_values(name, curso_id):
            return {"message": gettext("cadeira_name_exists").format(name)}, 400
